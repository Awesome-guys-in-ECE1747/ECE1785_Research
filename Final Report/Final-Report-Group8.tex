...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth]{QuantitativeResult.png}
...
All the participants are currently or formerly engineering occupied, and they may or may not share their experience in engineering related or unrelated divisions. For the further evaluation and analysis, we asked participants for permission to record audio in order to use transcribe services to convert the audio into text script. The classification for participants is based on their employment status. More detailedly, the students include one undergraduate student, 2 graduate student and one pH.D student. For junior engineers, one primary engineer works in SME and 3 of them work in large enterprise. All the senior engineer we interviewed are from large enterprises. For the reason of categorization, we will discuss further in the discussion section.
...
\\\\
We code the data based on the question we asked, and put the free talk into different sections, then we manually select key words and evaluate the information expressed in every section. There’re several findings we could conclude from the interview:​
